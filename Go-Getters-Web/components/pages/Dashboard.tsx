...
import { SupportCallModal } from "@/components/ui/SupportCallModal";
  ...
  useEffect(() => {
    if (currentUser) {
      try {
        ...
        // Only show support if they have been registered for more than 3 days (Onboarding grace period)
        if (
          !isNaN(diffDays) &&
          diffDays > 3 &&
        ...
        // Safe fallback in case of date parsing anomalies
        if (
          (currentUser.completionRate < 50 || currentUser.consistency < 50) &&
          (currentUser.completionRate > 0 || currentUser.consistency > 0)
        ) {
