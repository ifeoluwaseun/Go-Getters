...
const INITIAL_ACHIEVEMENTS: Achievement[] = [
  ...
  { id:'a5', type:'consistency', title:'Iron Will', description:'7-day perfect completion rate', icon:'shield', earnedAt:new Date(Date.now()-2419200000).toISOString(), color:'#00e57d' },
];
...
const INITIAL_TEAM_MEMBERS: TeamMember[] = [
  { id:'u3', name:'James T.', email:'james@example.com', role:'member', leaderId:'2', sponsorId:'2', streak:15, points:3890, completionRate:91, consistency:88, joinedAt:'2024-03-01', title:'Rising Star', lastActive:'2 hours ago', status:'active',
    tasks:[ {id:'mt1',title:'Morning Prospecting (10 contacts)',category:'Prospecting',dueTime:'09:00',priority:'high',status:'completed',hasEvidence:true,recurring:true,date:today,completedAt:new Date().toISOString()}, {id:'mt2',title:'Follow-up calls x5',category:'Follow-Up',dueTime:'11:00',priority:'high',status:'completed',hasEvidence:false,recurring:false,date:today,completedAt:new Date().toISOString()}, {id:'mt3',title:'Share daily win',category:'Content',dueTime:'16:00',priority:'medium',status:'pending',hasEvidence:false,recurring:true,date:today}, {id:'mt4',title:'Read leadership book',category:'Personal Dev',dueTime:'20:00',priority:'medium',status:'pending',hasEvidence:false,recurring:true,date:today} ],
    goals:[ {id:'mg1',title:'Close 5 new sales this week',description:'Focus on warm market leads',weekStart:today,category:'Sales',taskIds:['mt1','mt2'],progress:80,color:'#00d8fe'}, {id:'mg2',title:'Complete prospecting training',description:'Finish the online course modules',weekStart:today,category:'Growth',taskIds:['mt4'],progress:60,color:'#fbbf24'} ],
    evidence:[ {id:'me1',taskId:'mt1',taskTitle:'Morning Prospecting',type:'screenshot',description:'DMs sent to 12 prospects this morning',status:'approved',uploadedAt:new Date(Date.now()-3600000).toISOString(),userName:'James T.'} ],
  },
  { id:'u2', name:'Sarah K.', email:'sarah@example.com', role:'leader', leaderId:'1', sponsorId:'1', streak:18, points:4210, completionRate:95, consistency:92, joinedAt:'2024-02-10', title:'Team Leader', lastActive:'30 min ago', status:'active',
    tasks:[ {id:'st1',title:'Team accountability call',category:'Leadership',dueTime:'10:00',priority:'high',status:'completed',hasEvidence:true,recurring:true,date:today,completedAt:new Date().toISOString()}, {id:'st2',title:'Morning prospecting',category:'Prospecting',dueTime:'08:00',priority:'high',status:'completed',hasEvidence:true,recurring:true,date:today,completedAt:new Date().toISOString()}, {id:'st3',title:'Personal development session',category:'Personal Dev',dueTime:'13:00',priority:'medium',status:'pending',hasEvidence:false,recurring:true,date:today} ],
    goals:[ {id:'sg1',title:'Maintain team 90%+ completion',description:'Support all team members daily',weekStart:today,category:'Leadership',taskIds:['st1'],progress:95,color:'#a855f7'}, {id:'sg2',title:'Personal recruitment: 2 new members',description:'Add to warm market outreach',weekStart:today,category:'Recruitment',taskIds:['st2'],progress:50,color:'#00d8fe'} ],
    evidence:[ {id:'se1',taskId:'st1',taskTitle:'Team accountability call',type:'screenshot',description:"Recording link of today's team call",status:'approved',uploadedAt:new Date(Date.now()-1800000).toISOString(),userName:'Sarah K.'}, {id:'se2',taskId:'st2',taskTitle:'Morning prospecting',type:'link',description:'LinkedIn outreach log — 15 messages',status:'pending',uploadedAt:new Date(Date.now()-7200000).toISOString(),userName:'Sarah K.'} ],
  },
  { id:'u7', name:'Nina P.', email:'nina@example.com', role:'member', leaderId:'2', sponsorId:'2', streak:10, points:2950, completionRate:86, consistency:80, joinedAt:'2024-03-20', title:'Go-Getter', lastActive:'1 hour ago', status:'active',
    tasks:[ {id:'nt1',title:'Morning prospecting',category:'Prospecting',dueTime:'09:00',priority:'high',status:'completed',hasEvidence:true,recurring:true,date:today,completedAt:new Date().toISOString()}, {id:'nt2',title:'Product demo with client',category:'Sales',dueTime:'14:00',priority:'high',status:'pending',hasEvidence:false,recurring:false,date:today}, {id:'nt3',title:'Post testimonial',category:'Content',dueTime:'17:00',priority:'medium',status:'overdue',hasEvidence:false,recurring:false,date:today} ],
    goals:[ {id:'ng1',title:'Book 3 product demos',description:'Target existing warm market',weekStart:today,category:'Sales',taskIds:['nt2'],progress:33,color:'#00e57d'} ],
    evidence:[ {id:'ne1',taskId:'nt1',taskTitle:'Morning prospecting',type:'image',description:'Photo of my contact list for today',status:'pending',uploadedAt:new Date(Date.now()-5400000).toISOString(),userName:'Nina P.'} ],
  },
  { id:'u8', name:'Devon L.', email:'devon@example.com', role:'member', leaderId:'2', sponsorId:'2', streak:3, points:2700, completionRate:62, consistency:55, joinedAt:'2024-04-01', title:'Go-Getter', lastActive:'Yesterday', status:'at-risk',
    tasks:[ {id:'dt1',title:'Morning prospecting',category:'Prospecting',dueTime:'09:00',priority:'high',status:'overdue',hasEvidence:false,recurring:true,date:today}, {id:'dt2',title:'Follow-up calls',category:'Follow-Up',dueTime:'11:00',priority:'high',status:'overdue',hasEvidence:false,recurring:true,date:today}, {id:'dt3',title:'Evening review',category:'Planning',dueTime:'20:00',priority:'medium',status:'pending',hasEvidence:false,recurring:true,date:today} ],
    goals:[ {id:'dg1',title:'Get back on track this week',description:'Complete at least 5 tasks per day',weekStart:today,category:'Growth',taskIds:['dt1','dt2','dt3'],progress:20,color:'#ef4444'} ],
    evidence:[],
  },
  { id:'u9', name:'Chen W.', email:'chen@example.com', role:'member', leaderId:'2', sponsorId:'2', streak:7, points:2400, completionRate:79, consistency:74, joinedAt:'2024-04-10', title:'Go-Getter', lastActive:'4 hours ago', status:'active',
    tasks:[ {id:'ct1',title:'Morning prospecting',category:'Prospecting',dueTime:'09:00',priority:'high',status:'completed',hasEvidence:true,recurring:true,date:today,completedAt:new Date().toISOString()}, {id:'ct2',title:'Read personal dev book',category:'Personal Dev',dueTime:'12:00',priority:'medium',status:'completed',hasEvidence:false,recurring:true,date:today,completedAt:new Date().toISOString()}, {id:'ct3',title:'Team training session',category:'Leadership',dueTime:'15:00',priority:'high',status:'pending',hasEvidence:false,recurring:false,date:today} ],
    goals:[ {id:'cg1',title:'Build product knowledge',description:'Complete all training modules this week',weekStart:today,category:'Growth',taskIds:['ct2','ct3'],progress:55,color:'#a855f7'} ],
    evidence:[ {id:'ce1',taskId:'ct1',taskTitle:'Morning prospecting',type:'screenshot',description:'Screenshot of 8 new LinkedIn connections today',status:'approved',uploadedAt:new Date(Date.now()-14400000).toISOString(),userName:'Chen W.'} ],
  },
  { id:'u4', name:'Priya M.', email:'priya@example.com', role:'leader', leaderId:'1', sponsorId:'1', streak:14, points:3640, completionRate:94, consistency:90, joinedAt:'2024-02-20', title:'Team Leader', lastActive:'1 hour ago', status:'active',
    tasks:[ {id:'pt1',title:'Morning team briefing',category:'Leadership',dueTime:'08:30',priority:'high',status:'completed',hasEvidence:false,recurring:true,date:today,completedAt:new Date().toISOString()}, {id:'pt2',title:'Prospecting session',category:'Prospecting',dueTime:'10:00',priority:'high',status:'completed',hasEvidence:true,recurring:true,date:today,completedAt:new Date().toISOString()}, {id:'pt3',title:'Leadership podcast',category:'Personal Dev',dueTime:'18:00',priority:'low',status:'pending',hasEvidence:false,recurring:true,date:today} ],
    goals:[ {id:'pg1',title:'Lead team to 95% weekly completion',description:'Support every member to hit their targets',weekStart:today,category:'Leadership',taskIds:['pt1'],progress:90,color:'#a855f7'} ],
    evidence:[ {id:'pe1',taskId:'pt2',taskTitle:'Prospecting session',type:'screenshot',description:'10 warm outreach messages sent',status:'approved',uploadedAt:new Date(Date.now()-3600000).toISOString(),userName:'Priya M.'} ],
  },
];

const INITIAL_TEAM_MESSAGES: Record<string, TeamMessage[]> = {
  'u3': [ {id:'tm1',memberId:'u3',senderId:'2',senderName:'Marcus Johnson',content:'Great start to the week James! Keep that streak alive.',sentAt:new Date(Date.now()-86400000).toISOString(),type:'message'}, {id:'tm2',memberId:'u3',senderId:'2',senderName:'Marcus Johnson',content:'Remember to submit your evidence for the morning prospecting session once done.',sentAt:new Date(Date.now()-3600000).toISOString(),type:'reminder'} ],
  'u8': [ {id:'tm3',memberId:'u8',senderId:'2',senderName:'Marcus Johnson',content:"Devon, I noticed you have a couple of overdue tasks today. Let me know if you need support — I am here to help.",sentAt:new Date(Date.now()-7200000).toISOString(),type:'message'}, {id:'tm4',memberId:'u8',senderId:'2',senderName:'Marcus Johnson',content:"Consistency is built one day at a time. Let's get back on track together this week.",sentAt:new Date(Date.now()-1800000).toISOString(),type:'note'} ],
};
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const [teamMessages, setTeamMessages] = useState<Record<string, TeamMessage[]>>(INITIAL_TEAM_MESSAGES);

  const unreadCount = notifications.filter(n => !n.isRead).length;

  const generateId = () => Date.now().toString() + Math.random().toString(36).substr(2, 5);

  const completeTask = (id: string) => setTasks(prev => prev.map(t => t.id === id ? { ...t, status: 'completed', completedAt: new Date().toISOString() } : t));
  ...
  const likePost = (id: string) => setPosts(prev => prev.map(p => p.id === id ? { ...p, liked: !p.liked, likes: p.liked ? p.likes - 1 : p.likes + 1 } : p));
  const addPost = (content: string, type: Post['type']) => {
    // Should pass user details realistically from Auth, mocked here since AuthContext exists elsewhere
    setPosts(prev => [{
      id: generateId(),
      userId: 'u-self', // will be replaced in component if needed
      userName: 'You',
      userRole: 'member',
      content,
      type,
  ...
  const addEvidence = (ev: Omit<Evidence, 'id'>) => {
    setEvidence(prev => [...prev, { ...ev, id: generateId() }]);
    setTasks(prev => prev.map(t => t.id === ev.taskId ? { ...t, hasEvidence: true } : t));
  };
  const approveEvidence = (id: string) => setEvidence(prev => prev.map(e => e.id === id ? { ...e, status: 'approved' } : e));
  const rejectEvidence = (id: string, feedback: string) => setEvidence(prev => prev.map(e => e.id === id ? { ...e, status: 'rejected', feedback } : e));
  const sendTeamMessage = (memberId: string, content: string, senderId: string, senderName: string, type: TeamMessage['type'] = 'message') => {
    setTeamMessages(prev => {
      const msgs = prev[memberId] || [];
      return {
        ...prev,
        [memberId]: [...msgs, { id: generateId(), memberId, senderId, senderName, content, type, sentAt: new Date().toISOString() }]
      };
    });
  };
  ...
  return (
    <AppContext.Provider value={{
      tasks, goals, posts, leaderboard, meetings, notifications, achievements, achievers, evidence, teamMembers, teamMessages, unreadCount,
      completeTask, addTask, addGoal, likePost, addPost, markNotificationRead, markAllRead, addEvidence, approveEvidence, rejectEvidence, sendTeamMessage
    }}>
      {children}
    </AppContext.Provider>
  );
